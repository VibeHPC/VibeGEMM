// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M13312_N13312_K13312_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M13312_N13312_K13312_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M13312_N13312_K13312_FP16FP16FP32FP16_H100
                    ...
                    if constexpr (CLUSTER_N_ > 1) {
                        ...
                        if (rank_n == 0) {
                            tmaFetchAsyncMulticast(&sA[qidx*BK*BM], &tensorMapA, &full[qidx],
                                                 block_k_iter*BK, num_block_m*BM, (uint16_t)mask);
                        }
                    } else {
                    ...
                    if constexpr (CLUSTER_M_ > 1) {
                        if (rank_m == 0) {
                            tmaFetchAsyncMulticast(&sB[qidx*BK*BN], &tensorMapB, &full[qidx],
                                                 block_k_iter*BK, num_block_n*BN, b_mask);
                        }
                    } else {
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=13312, warmup_iters=10, benchmark_iters=30\n");
}
