// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M1280_N1280_K1280_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M1280_N1280_K1280_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M1280_N1280_K1280_FP16FP16FP32FP16_H100
...
__device__ static __forceinline__ void initBarrier(uint64_t *bar, int thread_count,
...
__device__ static __forceinline__ void mbarrierExpectTx(uint64_t *bar, uint32_t bytes) {
    uint32_t ptr = smemPtrU32(bar);
    asm volatile(
...
__device__ static __forceinline__ void mbarrierWait(uint64_t *bar, int phase_bit) {
    uint32_t ptr = smemPtrU32(bar);
    asm volatile(
        "{\n"
        ".reg .pred P1;\n"
...
__device__ void arriveCluster(uint64_t *bar, uint32_t cta_id, uint32_t count = 1) {
...
template <int BM, int BN, int BK, int QSIZE> struct SingleBufferSmem {
...
tmaFetchAsyncMulticast(half *dst, void const *const src_tma, uint64_t *bar, int global_col_idx,
...
__device__ __forceinline__ void tmaLoad3d(half *smem_ptr, const CUtensorMap *map,
                                          int coord_k64, int coord_mn, uint64_t *bar) {
...
template <int BlockMajorSize, int BlockMinorSize, bool swizzle = true, bool padded = false>
__host__ static inline CUtensorMap createTmaMapDevice(const half *gmem_ptr, int global_height,
                                                      int global_width) {
    CUtensorMap tma_map;
    ...
    uint64_t gmem_prob_shape[5] = {
        64, (uint64_t)global_height, (uint64_t)global_width / 64, 1, 1
    };
    uint64_t gmem_prob_stride[5] = {
        sizeof(half) * global_width, 64 * sizeof(half), 0, 0, 0
    };
...
__device__ static inline void tmaFetchAsync(half *dst, void const *src_tma_map, uint64_t *bar,
...
template <int BM, int BN, int BK, int NUM_THREADS, int QSIZE, int NUM_SM, int CLUSTER_M_,
          int CLUSTER_N_>
__global__ __launch_bounds__(NUM_THREADS) void __cluster_dims__(CLUSTER_M_ * CLUSTER_N_, 1, 1)
    gemm(int M, int N, int K, const __grid_constant__ CUtensorMap tensorMapC,
         ...
         const __grid_constant__ CUtensorMap tensorMapB, const int *__restrict__ dspace) {
            ...
            while (schedule.next(bIdxM, bIdxN)) {
                ...
                for (int kStep = 0; kStep < numBlocksK; ++kStep, ++qIdx) {
                    ...
                    if constexpr (CLUSTER_N_ > 1) {
                        uint32_t mask = ((1u << CLUSTER_N_) - 1u) << (rankM * CLUSTER_N_);
                        if (rankN == 0) {
                            tmaFetchAsyncMulticast(&sA[qIdx * BK * BM], &tensorMapA,
                    ...
                    if constexpr (CLUSTER_M_ > 1) {
                        if (rankM == 0) {
                            tmaFetchAsyncMulticast(&sB[qIdx * BK * BN], &tensorMapB,
...
#pragma unroll
                for (int mIt = 0; mIt < B_WG_M / WGMMA_M; ++mIt) {
                    half *wgmmaSA =
                        sA + qIdx * BK * BM + 64 * (mIt + wgIdx * B_WG_M / WGMMA_M) * WGMMA_M;
                    half *wgmmaSB = sB + qIdx * BK * BN;

                    wgmma<WGMMA_N, 0, 1, 1, 0, 0>(d[mIt], &wgmmaSA[0], &wgmmaSB[0]);
#pragma unroll
...
#pragma unroll
                for (int w = 0; w < WGMMA_N; w += 16) {
                    uint32_t addr =
                        baseAddr + (w * B_WG_M_PADDED + mIt * WGMMA_M) * sizeof(half);
...
#pragma unroll
                    ...
                    asm volatile(
                        ...
                        :
                        : "r"(addr),
                          "r"(data_ptr[0]), "r"(data_ptr[1]), "r"(data_ptr[2]),
                          "r"(data_ptr[3]));
                }
            }
            ...
            if (tid == 0) {
                ...
                storeAsync(&tensorMapC, blockSC, bIdxM * BM + wgIdx * B_WG_M, bIdxN * BN);
...
static int *generateHilbertHostData(int m_tiles, int n_tiles, int num_clusters) {
    ...
    for (int i = 0; i < num_clusters * depth; ++i)
        h[i] = -1;
...
static void launch_custom_gemm(const half *d_A, const half *d_B, half *d_C, int M, int N, int K) {
    constexpr int BM = 128, BN = 192, BK = 64, QSIZE = 3, NUM_THREADS = 384, NUM_SM = 128;
    ...
    if (prevMBase != M || prevNBase != N || prevKBase != K) {
        ...
        globalTmaMapCBase = createTmaMapDevice<BN, BM / num_consumers, false, true>(d_C, N, M);
        prevMBase = M;
        prevNBase = N;
        prevKBase = K;
    }
    ...
    kptr<<<NUM_SM, NUM_THREADS, sharedBytes>>>(M, N, K, globalTmaMapCBase,
                                               globalTmaMapABase, globalTmaMapBBase,
                                               globalDspaceBase);
}
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=1280, warmup_iters=10, benchmark_iters=30\n");
}
