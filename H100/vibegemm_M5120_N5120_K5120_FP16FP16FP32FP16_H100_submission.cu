// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M5120_N5120_K5120_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M5120_N5120_K5120_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M5120_N5120_K5120_FP16FP16FP32FP16_H100
...
__global__ __launch_bounds__(NUM_THREADS) void __cluster_dims__(
    ...
    if (wg_idx == 0) {
        ...
        if (tid == 0) {
            ...
            int qidx = 0;
            uint32_t col_mask = 0;
            for (int i = 0; i < CLUSTER_M_; ++i)
                col_mask |= (1 << (i * CLUSTER_N_));
...
#define ST(r_off, c_off, val)                                                  \
    block_sC[(col_offset + (c_off)) * B_WG_M + (row_offset + row + (r_off))] = \
        (half)(val)
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=5120, warmup_iters=10, benchmark_iters=30\n");
}
