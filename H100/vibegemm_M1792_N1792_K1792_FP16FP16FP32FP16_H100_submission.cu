// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M1792_N1792_K1792_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M1792_N1792_K1792_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M1792_N1792_K1792_FP16FP16FP32FP16_H100
...
__device__ uint64_t makeSmemDescriptor(half *ptr) {
    ...
    uint64_t desc = encodeMatrixDescriptor(addr);
...
__device__ __forceinline__ void wgmmaM64N256K16(float d[16][8], half *sA,
                                                half *sB) {
...
template <int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ __forceinline__ void wgmmaM64N192K16(float d[12][8], half *sA,
                                                half *sB) {
    uint64_t desc_a = makeSmemDescriptor(&sA[0]);
    uint64_t desc_b = makeSmemDescriptor(&sB[0]);
    asm volatile(
...
__device__ __forceinline__ void wgmma(float d[WGMMA_N / 16][8], half *sA,
    ...
    if constexpr (WGMMA_N == 256)
        wgmmaM64N256K16<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 192)
        wgmmaM64N192K16<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
...
struct SingleBufferSmem {
    ...
    alignas(128) half C[BN * (BM + (BM / 64) * 8)];
...
__device__ static inline void tmaFetchAsyncMulticast(
    half *dst, void const *src_tma_map, uint64_t *bar, int global_col_idx,
...
__global__ __launch_bounds__(NUM_THREADS) void __cluster_dims__(
    ...
    gemm(int M, int N, int K, const __grid_constant__ CUtensorMap tensorMapC,
         ...
         const __grid_constant__ CUtensorMap tensorMapB, int *dspace) {
    ...
    Schedule<2, NUM_SM / CLUSTERS, BM * CLUSTER_M, BN * CLUSTER_N,
...
static void launch_custom_gemm(const half *d_A, const half *d_B, half *d_C,
    ...
    constexpr size_t sMemSize = sizeof(SingleBufferSmem<BM, BN, BK, QSIZE>);
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=1792, warmup_iters=10, benchmark_iters=30\n");
}
