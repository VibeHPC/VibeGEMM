// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M10240_N10240_K10240_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M10240_N10240_K10240_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M10240_N10240_K10240_FP16FP16FP32FP16_H100
...
__device__ static inline void tmaFetchAsyncMulticast(
    ...
    asm volatile(
        ...
        "complete_"
        "tx::bytes.multicast::cluster [%0], [%1, {%3, %4, %5}], [%2], %6;"
        :
...
struct Schedule<1, NUM_SM, BM, BN, TM, TN> {
    ...
    __device__ __forceinline__ bool next(int &block_m, int& block_n) {
        ...
        int total = total_blocks_m * total_blocks_n;
        if (total <= 0) return false;

        int tiles_m = (total_blocks_m + kTM - 1) / kTM;
        int tiles_n = (total_blocks_n + kTN - 1) / kTN;
        int tiles_total = tiles_m * tiles_n;
        int virtual_total = tiles_total * kTM * kTN;

        int num = it * NUM_SM + block;
        if (num >= virtual_total) return false;

        while (num < virtual_total) {
            int cur_tile = num / (kTM * kTN);
            int cur_pos = num - cur_tile * (kTM * kTN);

            int tile_m = cur_tile / tiles_n;
            int tile_n = cur_tile - tile_m * tiles_n;

            int m0 = tile_m * kTM;
            int n0 = tile_n * kTN;

            int dm = cur_pos / kTN;
            int dn = cur_pos - dm * kTN;

            block_m = m0 + dm;
            block_n = n0 + dn;
            ++it;

            if (block_m < total_blocks_m && block_n < total_blocks_n) {
                return true;
            }
...
__global__  __launch_bounds__(NUM_THREADS) void  __cluster_dims__(CLUSTER_M * CLUSTER_N, 1, 1) gemm(int M, int N, int K, const __grid_constant__ CUtensorMap tensorMapC, const __grid_constant__ CUtensorMap tensorMapA, const __grid_constant__ CUtensorMap tensorMapB) {
    ...
    const uint32_t cluster_id = blockIdx.x / CLUSTERS;
    const uint32_t cluster_count = gridDim.x / CLUSTERS;
    ...
    (void)cluster_count;
    Schedule<1, NUM_SM/CLUSTERS, BM*CLUSTER_M, BN*CLUSTER_N, 16/CLUSTER_M, 8/CLUSTER_N> schedule(M, N, cluster_id);

    uint32_t cta_rank;
    asm volatile("mov.u32 %0, %cluster_ctarank;\n" : "=r"(cta_rank) :);
    uint32_t rank_m = cta_rank / CLUSTER_N;
    uint32_t rank_n = cta_rank % CLUSTER_N;
            ...
            while (schedule.next(num_block_m, num_block_n)) {
                ...
                for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                    ...
                    if constexpr (CLUSTER_N > 1) {
                        ...
                        if (rank_n == 0) {
                            tmaFetchAsyncMulticast(&sA[qidx*BK*BM], &tensorMapA, &full[qidx], block_k_iter*BK, num_block_m*BM, mask);
                        }
                    } else {
                        tmaFetchAsync(&sA[qidx*BK*BM], &tensorMapA, &full[qidx], block_k_iter*BK, num_block_m*BM);
                    }

                    if constexpr (CLUSTER_M > 1) {
                        if (rank_m == 0) {
                            tmaFetchAsyncMulticast(&sB[qidx*BK*BN], &tensorMapB, &full[qidx], block_k_iter*BK, num_block_n*BN, col_mask << rank_n);
                        }
                    } else {
                        tmaFetchAsync(&sB[qidx*BK*BN], &tensorMapB, &full[qidx], block_k_iter*BK, num_block_n*BN);
                    }
                }
            }
        }
...
static void launch_custom_gemm(const half *d_A, const half *d_B, half *d_C, int M, int N, int K) {
    ...
    CUDA_CHECK(cudaFuncSetAttribute(kptr, cudaFuncAttributeMaxDynamicSharedMemorySize,
                         (int)sharedBytes));
    kptr<<<NUM_SM, NUM_THREADS, sharedBytes>>>(M, N, K, globalTmaMapCBase, globalTmaMapABase, globalTmaMapBBase);
}
// Benchmark wrapper
// ========================================================================================
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=10240, warmup_iters=10, benchmark_iters=30\n");
}
