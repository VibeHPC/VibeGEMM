// Compile Command:
// nvcc -O3 -std=c++17 -gencode arch=compute_90a,code=sm_90a --expt-relaxed-constexpr --expt-extended-lambda vibegemm_M14080_N14080_K14080_FP16FP16FP32FP16_H100_submission.cu -lcublas -lcuda -o vibegemm_M14080_N14080_K14080_FP16FP16FP32FP16_H100
//
// Run Command:
// ./vibegemm_M14080_N14080_K14080_FP16FP16FP32FP16_H100
...
__host__ CUtensorMap createTmaMapDevice(const half *src, int globalHeight, int globalWidth) {
    CUtensorMap hostMap;
    static_assert(BlockMinorSize >= 64);
    assert(globalWidth % 64 == 0);

    uint64_t shape[5] = {64, (uint64_t)globalHeight, (uint64_t)globalWidth / 64, 1, 1};
    uint64_t stride[5] = {sizeof(half) * globalWidth, 64 * sizeof(half), 0, 0, 0};
    uint32_t smemShape[5] = {64, uint32_t(BlockMajorSize), uint32_t(BlockMinorSize / 64), 1, 1};
    uint32_t smemStride[5] = {1, 1, 1, 1, 1};

    CUresult result = cuTensorMapEncodeTiled(
        &hostMap, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 3, (void *)src, shape, stride, smemShape,
        smemStride, CU_TENSOR_MAP_INTERLEAVE_NONE,
...
__device__ __forceinline__ void tmaFetchAsync(half *dst, void const *map, uint64_t *bar,
...
__device__ __forceinline__ void tmaFetchAsyncMulticast(half *dst, void const *map, uint64_t *bar,
                                                       int globalColIdx, int globalRowIdx,
                                                       uint16_t clusterMask) {
    uint64_t tma_ptr = reinterpret_cast<uint64_t>(map);
    uint32_t bar_ptr = smemPtrU32(bar);
    uint32_t dst_ptr = smemPtrU32(dst);
...
__device__ __forceinline__ void tmaStoreAsync(void const *map, half *src, int globalColIdx,
                                              int globalRowIdx) {
    uint64_t tma_ptr = reinterpret_cast<uint64_t>(map);
            ...
            while (schedule.next(num_block_m, num_block_n)) {
                ...
                for (int kStep = 0; kStep < numBlocksK; ++kStep, ++qidx) {
                    ...
                    if constexpr (CLUSTER_N_ > 1) {
                        uint32_t mask = ((1u << CLUSTER_N_) - 1u) << (rank_m * CLUSTER_N_);
                        if (rank_n == 0) {
                            tmaFetchAsyncMulticast(&sA[qidx * BK * BM], &tmaA, &full[qidx],
                                                   kStep * BK, num_block_m * BM,
                                                   (uint16_t)mask);
                        }
                    } else {
                        tmaFetchAsync(&sA[qidx * BK * BM], &tmaA, &full[qidx], kStep * BK,
                                      num_block_m * BM);
                    }

                    if constexpr (CLUSTER_M_ > 1) {
                        if (rank_m == 0) {
                            tmaFetchAsyncMulticast(&sB[qidx * BK * BN], &tmaB, &full[qidx],
                                                   kStep * BK, num_block_n * BN, b_mask);
                        }
                    } else {
                        tmaFetchAsync(&sB[qidx * BK * BN], &tmaB, &full[qidx], kStep * BK,
                                      num_block_n * BN);
                    }
                }
            }
...
#pragma unroll
                for (int mi = 0; mi < BWgM / WGMMA_M; ++mi) {
                    half *ptrA =
                        sA + qidx * BK * BM + 64 * (mi + wg_idx * BWgM / WGMMA_M) * WGMMA_M;
...
#pragma unroll
                    for (int ki = 1; ki < 64 / WGMMA_K; ++ki) {
                        wgmma<WGMMA_N, 1, 1, 1, 0, 0>(d[mi], &ptrA[ki * WGMMA_K],
                                                       &ptrB[ki * WGMMA_K]);
                    }
...
#define ST(r_off, c_off, val) \
    block_sC[(col_offset + (c_off)) * BWgM + (row_offset + row + (r_off))] = __float2half(val)
...
static int *generateHilbertHostData(int mTiles, int nTiles, int numClusters) {
    ...
    for (int d = 0; d < maxD && taskId < totalTasks; ++d) {
        ...
        if (y < mTiles && x < nTiles) {
            ...
            if (pos < depth) {
                hostSpace[cluster * depth + pos] = (block_m << 16) | (block_n & 0xFFFF);
            }
...
static void launch_custom_gemm(const half *d_A, const half *d_B, half *d_C, int M, int N, int K) {
    ...
    const bool need_regen = (_dspace == nullptr) || (_space_m_tiles != m_tiles) ||
    ...
    if (need_regen) {
        ...
        int *hostSpace = generateHilbertHostData(m_tiles, n_tiles, NUM_CLUSTERS);
    ...
    auto kernel = &matmulKernel3Pipeline<BM, BN, BK, NUM_THREADS, QSIZE, NUM_SM, CLUSTER_M,
                                         CLUSTER_N>;

    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)sharedBytes);

    kernel<<<NUM_SM, NUM_THREADS, sharedBytes>>>(M, N, K, tmaC, tmaA, tmaB, _dspace);
}// Benchmark wrapper
// ========================================================================================
...
static void print_usage(const char *program) {
    ...
    std::printf("\nDefaults:\n");
    std::printf("  M=N=K=14080, warmup_iters=10, benchmark_iters=30\n");
}
